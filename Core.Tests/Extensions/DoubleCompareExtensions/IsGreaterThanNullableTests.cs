...
/// </summary>
internal sealed class IsGreaterThanNullableTests
{
	#region Sources
	...
	public void ShouldHandleNullValues( double? value )
	{
		...
		Assert.Multiple( () =>
		{
			Assert.That( nullValue.IsGreaterThan( nullValue ), Is.False );
			Assert.That( nullValue.IsGreaterThan( value ), Is.False );
			Assert.That( value.IsGreaterThan( nullValue ), Is.False );
		} );
	}
