...
/// </summary>
internal sealed class IsGreaterThanOrEqualToNullableTests
{
	#region Sources
	...
	public void ShouldHandleNullValues( double? value )
	{
		...
		Assert.Multiple( () =>
		{
			Assert.That( nullValue.IsGreaterThanOrEqualTo( nullValue ), Is.False );
			Assert.That( nullValue.IsGreaterThanOrEqualTo( value ), Is.False );
			Assert.That( value.IsGreaterThanOrEqualTo( nullValue ), Is.False );
		} );
	}
